...
* `analysis`：全文显示题目解析（如果有解析的话），优先级等于题组环境 `qitems` 的显示解析选项 `showanalysis`（见 [题目录入环境及命令](#题目录入环境及命令)），二者不冲突，可灵活使用
...
1. 题组环境
   ...
   * `showanalysis`：题组环境内显示题目解析，优先级等于文档类选项 `analysis`（见 [文档类选项](#文档类选项)），二者不冲突，可灵活使用
2. 题目环境
   ```latex
   \begin{bbox}
